...
import { fetchAccounts, createAccount, updateAccount } from '../api'
...
export default function TeacherAccounts() {
    ...
    async function loadAccounts() {
        try {
            const data = await fetchAccounts('?account_type=teacher')
    ...
    async function handleSubmit(e) {
        ...
        if (!form.account_id || !form.name) {
            toast.warning('Staff ID and Name are required')
            return
        }
    ...
    return (
        <div>
            <PageHeader
                ...
                subtitle={`${accounts.length} teacher accounts`}
                actions={canEdit && <Button onClick={openAddModal}>+ Add Teacher</Button>}
            />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 'var(--spacing-4)' }}>
                {filteredAccounts.map(account => (
                    ...
                    >
                        <div className={styles.accountAvatar} style={{ background: 'var(--color-success)' }}>
            ...
            <Modal
                ...
                title={editingAccount ? 'Edit Teacher Account' : 'Add Teacher Account'}
                footer={
                    <>
